...
void CWndShadow::Register(HINSTANCE hInstance)
{
	// Register window class for shadow window
	WNDCLASSEX wcex;
...
bool CWndShadow::Create(HINSTANCE hInstance, HWND hParentWnd)
{
	// Do nothing if the system does not support layered windows
	if(NULL == s_UpdateLayeredWindow)
		return false;

	if(NULL == hParentWnd) return false;

	s_hInstance = hInstance;
	Register(s_hInstance);
	// Already initialized
