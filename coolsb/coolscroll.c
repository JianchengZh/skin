...
#if PARENT_NOTIFY
	hwnd = GetParent(hwnd);// 해당 컨트롤로 직접 메세지를 보낸다.
#endif
...
static LRESULT NCLButtonDown(SCROLLWND *sw, HWND hwnd, WPARAM wParam, LPARAM lParam)
{
	...
	case HTSCROLL_PAGELEFT:  case HTSCROLL_PAGERIGHT:
		...
		uMouseDownId = SetTimer(hwnd, COOLSB_TIMERID4, COOLSB_TIMERINTERVAL4, 0);
...
static LRESULT LButtonUp(SCROLLWND *sw, HWND hwnd, WPARAM wParam, LPARAM lParam)
{
	...
	return 0;//CallWindowProc(sw->oldproc, hwnd, WM_LBUTTONUP, wParam, lParam);
...
static LRESULT MouseMove(SCROLLWND *sw, HWND hwnd, WPARAM wParam, LPARAM lParam)
{
	...
	else
	{
		...
		pt.x = GET_X_LPARAM(lParam);//LOWORD(nlParam);
		pt.y = GET_Y_LPARAM(lParam);//HIWORD(nlParam);
...
static LRESULT CoolSB_Timer(SCROLLWND *swnd, HWND hwnd, WPARAM wTimerId, LPARAM lParam)
{
	...
	else if(wTimerId == COOLSB_TIMERID4)//mouse down
	{
		SendScrollMessage(hwnd, uScrollTimerMsg, uCurrentScrollPortion, 0);
	}	
	else
...
{
#ifdef INCLUDE_BUTTONS
	UINT lo = GET_X_LPARAM(lParam);//LOWORD(lParam);
	UINT hi = GET_Y_LPARAM(lParam);//HIWORD(lParam);
	UINT xy;
	RECT rect;
	SCROLLBAR *sbar;
	SCROLLBUT *sbut;
	...
	{
		xy = GetMessagePos();
		pt.x = GET_X_LPARAM(xy);//LOWORD(xy);
		pt.y = GET_Y_LPARAM(xy);//HIWORD(xy);
